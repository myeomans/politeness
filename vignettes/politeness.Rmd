...
Finally, our tools also have (deprecated) compatibility with [coreNLP](https://stanfordnlp.github.io/CoreNLP/) for part-of-speech tagging - however we do not recommend this at present, because the system requirements can be quite onerous for new users.
...
df_politeness_count <- politeness(phone_offers$message, binary=FALSE, num_mc_cores=2)
df_politeness_count[20:30,1:5]
```

```{r, echo=TRUE, eval=TRUE}
df_politeness <- politeness(phone_offers$message, binary=TRUE, num_mc_cores=2)
...
Often some features are too rare to be meaningful for further analysis, so users have an option to exclude them from the plot using the `drop_blank` parameter. This must be a number between 0 and 1, and determines a cut-off such that all features which appear in less than this proportion of texts are excluded from the plot. To include all features, simply set this value to `0`. 
...
This function is a wrapper around \code{texitr}, which implements a massively multinomial inverse regression. Intuitively, this model represents a more realistic causal structure to text-plus-covariate data - that is, the covariate typically has a causal effect on the words used by the speaker, rather than the other way around. Additioally, this package is computationally efficient, and this can be impvoed by registering a parallel back-end and supplying it to `mnlm_cluster` (see \code{texitr} documentation for details).
...
df_polite_train <- politeness(phone_offers$message, drop_blank=FALSE, num_mc_cores=2)

df_polite_holdout<-politeness(bowl_offers$message, drop_blank=FALSE, num_mc_cores=2)
