...
Often some features are too rare to be meaningful for further analysis, so users have an option to exclude them from the plot using the `drop.blank` parameter. This must be a number between 0 and 1, and determines a cut-off such that all features which appear in less than this proportion of texts are excluded from the plot. To include all features, simply set this value to `0`. 
...
df_polite_train <- politeness(phone_offers$message, drop.blank=FALSE)

df_polite_holdout<-politeness(bowl_offers$message, drop.blank=FALSE)
