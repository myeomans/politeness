...
Both packages have their merits, though for now we recommend using \code{glmnet} to start, especialy if it is familiar. Both algorithms are computationally efficient, and can be improved by registering a parallel back-end (see package details) and indicating in `cluster`.
...
## Execution time for different number of texts

In order go gauge the execution time of `politeness`, we ran it with different number of texts of different lengths. These tests where ran on a 2016 Macbook Pro with 2.7 GHz Intel Core i7. 

The figure below shows the execution time in minutes for 100, 1,000 and 10,000 length vector of texts of  100 and 200 number of words. For each case we rant it five times with parser `none` and `spacy`.

```{r,  out.width = "650px"}
knitr::include_graphics("executionTime.png")
```

We observe that `politeness` scales approximately linearly. For example, given texts of 200 words and `spacy` parser, we observe that 1,000 and 10,000 length vectors of texts take an average of 0.54 and 5.8 minutes, respectively. 
